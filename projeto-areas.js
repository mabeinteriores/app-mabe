...
var responsibleLabel=e('label','Responsável'),responsible=e('select');responsible.name='responsible';responsible.required=true;responsibleLabel.append(responsible);form.append(responsibleLabel);
function eligiblePeople(){return CamberResponsaveisObra.list(window.CamberCloud&&CamberCloud.usuariosObra?CamberCloud.usuariosObra():[],CamberDB.loadFornecedores?CamberDB.loadFornecedores():[]);}
function fillPeople(record){var current=record?record.responsibleKey:responsible.value,people=eligiblePeople();responsible.replaceChildren(new Option('Selecione um responsável',''));[['usuario','Usuários do sistema'],['terceiro','Terceiros habilitados']].forEach(function(group){var opt=e('optgroup');opt.label=group[1];people.filter(function(p){return p.type===group[0];}).forEach(function(p){opt.append(new Option(p.name,p.key));});if(opt.children.length)responsible.append(opt);});if(record&&!current&&record.responsible){var matches=people.filter(function(p){return p.name===record.responsible;});if(matches.length===1)current=matches[0].key;}responsible.value=current||'';if(record&&record.responsible&&!responsible.value)responsible.options[0].textContent='Selecione novamente (anterior: '+record.responsible+')';}
window.addEventListener('camber-responsaveis',function(){fillPeople();});fillPeople();
var start=field('Início','start','date'),end=field('Fim previsto','end','date');var label=e('label','Status'),select=e('select');['A iniciar','Em andamento','Pausada','Concluído'].forEach(function(v){select.add(new Option(v,v));});select.disabled=true;label.append(select);form.append(label);var save=e('button','Adicionar etapa');save.className='btn primary';save.type='submit';var cancel=e('button','Cancelar edição');cancel.type='button';cancel.className='btn';cancel.hidden=true;form.append(save,cancel);var status=e('p');status.setAttribute('role','status');var list=e('div');box.append(form,status,list);wrap.append(box);var editing=null,busy=false;
function rows(){return (CamberDB.getProject(projId)||{}).cronograma||[];}
function reset(){editing=null;form.reset();fillStages();fillPeople();save.textContent='Adicionar etapa';cancel.hidden=true;}
...
form.onsubmit=async function(ev){
 ...
 var chosen=eligiblePeople().find(function(p){return p.key===responsible.value;});if(!chosen){status.textContent='Selecione um usuário cadastrado ou terceiro habilitado.';responsible.focus();return;}
 ...
 try{
  ...
  var data=rows().slice(),previous=data.find(function(r){return r.id===id;}),record=Object.assign({},previous||{},{id:id,title:name.value.trim(),responsible:chosen.name,responsibleKey:chosen.key,responsibleId:chosen.id,responsibleType:chosen.type,start:start.value,end:end.value,status:previous?previous.status:'A iniciar'});
...
function markLate(host,task){var text=CamberAgenda.lateLabel(task);if(!text)return;host.classList.add('schedule-late');var badge=e('span',text);badge.className='schedule-late-badge';host.append(badge);}
var lateSummary=e('div');lateSummary.className='schedule-late-summary';lateSummary.setAttribute('role','status');box.before(lateSummary);
...
draw=function(){
 ...
 data.forEach(function(r){var row=e('div');row.style.cssText='display:grid;grid-template-columns:minmax(130px,220px) 1fr;gap:18px;padding:16px 0;border-top:1px solid var(--hair);align-items:center';var label=e('button',r.title);label.className='btn';markLate(label,r);label.onclick=function(){editor(r);};var track=e('div');track.style.cssText='height:28px;background:var(--surface-2);border-radius:6px;position:relative';var bar=e('div');bar.title=r.start+' — '+r.end+' · '+r.status;bar.style.cssText='height:100%;position:absolute;border-radius:6px;background:'+(CamberAgenda.lateDays(r)>0?'#b42318':r.status==='Concluído'?'#849273':'#bd7952')+';left:'+((day(r.start)-min)/one/span*100)+'%;width:'+((Math.round((day(r.end)-day(r.start))/one)+1)/span*100)+'%';track.append(bar);row.append(label,track);list.append(row);});return;}
